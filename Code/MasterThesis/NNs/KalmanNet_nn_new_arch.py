...
class KalmanNetNN(torch.nn.Module):
    ...
    def Build(self, ssModel):
        ...
        self.InitSequence(ssModel.m1x_0)#, SysModel.m2x_0, SysModel.T)
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S):
        ...
        self.h_Q = torch.randn(self.seq_len_input, self.batch_size, self.d_hidden_Q).to(self.dev , non_blocking=True)

        # GRU to track Sigma
        self.d_input_Sigma = self.d_hidden_Q + self.m * in_mult
        self.d_hidden_Sigma = self.m ** 2
        self.GRU_Sigma = nn.GRU(self.d_input_Sigma, self.d_hidden_Sigma)
        self.h_Sigma = torch.randn(self.seq_len_input, self.batch_size, self.d_hidden_Sigma).to(self.dev , non_blocking=True)

        # GRU to track S
        self.d_input_S = self.n ** 2 + 2 * self.n * in_mult
        self.d_hidden_S = self.n ** 2
        self.GRU_S = nn.GRU(self.d_input_S, self.d_hidden_S)
        self.h_S = torch.randn(self.seq_len_input, self.batch_size, self.d_hidden_S).to(self.dev , non_blocking=True)
        ...
        torch.nn.init.constant_(self.FC2[2].weight,0)
        torch.nn.init.constant_(self.FC2[2].bias,0)
    ...
    def InitSequence(self, M1_0):
        ...
        self.m1x_posterior = torch.squeeze(M1_0).to(self.dev , non_blocking=True)
        self.m1x_posterior_previous = self.m1x_posterior.to(self.dev , non_blocking=True)
        self.m1x_prior_previous = self.m1x_posterior.to(self.dev , non_blocking=True)
        self.y_previous = self.h(self.m1x_posterior,0).to(self.dev , non_blocking=True)
    ...
    def KNet_step(self, y,t):
        ...
        self.m1x_posterior = self.m1x_prior + torch.bmm(self.KGain, dy).squeeze()
    ...
    def forward(self, y,t):
        y = torch.squeeze(y).to(self.dev , non_blocking=True)
